...
const RUNES = {
  ...
  // exclusivas del modo infinito; cada copia da un bono permanente (hasta 10)
  Void:     { id:"Void",     name:"Runa del Vacío",     desc:"+5% de daño por copia (máx. 10) · solo en el modo infinito, oleada 20+", glyph:"🕳" },
  Eternity: { id:"Eternity", name:"Runa de la Eternidad",desc:"+5% de gemas por copia (máx. 10) · solo en el modo infinito, oleada 30+", glyph:"♾" },
};
...
const LOOK_DEFAULT = { outfit:"hunter", armor:"light", hair:"hood", hairColor:"#15121f", eyes:"#4a5a78", cape:"none", aura:"none" };
function needMet(n){
  if (!n) return true;
...
function claimExpedition(euid){
  ...
  let extra = "";
  if (ok && Math.random() < x.rune){
    const r = ["Health", "Gems", "Cash", "Time"][Math.floor(Math.random() * 4)];
    P.runes[r] = (P.runes[r] || 0) + 1; extra = ` · ${RUNES[r].name}`;
  }
...
function panelExpeditions(nav){
  ...
  if (x && expUnlocked(x)){
    ...
    pick = `<div class="sheet" style="--tc:var(--monarch)"><b class="sname">${x.icon} ${x.name}</b>
      <span class="lv">${x.hours} horas · poder recomendado ${fmt(x.need)} · recompensa ${fmt(x.cash * (1 + P.level * .01))} oro, ${fmt(x.gems * (1 + P.level * .01))} gemas${x.tickets ? `, ${x.tickets} tickets` : ""}${x.rune ? `, ${Math.round(x.rune * 100)}% de runa` : ""}</span>
...
const baseDamage  = () => FORMULA.physicalDamage(weaponDmg(), P.stats.STR) * (blessed() ? 1.4 : 1) * (formActive() ? FORM.dmg : 1)
  ...
  * (1 + Math.min(10, P.runes?.Void || 0) * .05);
...
const maxHP       = () => FORMULA.maxHP(100 + P.level * 6 + P.rebirths * 500, P.stats.VIT) * (1 + lookBonus("hp"))
  * (1 + classBonus("hp") + talentLv("vigor") * 0.1);
const maxMana     = () => FORMULA.maxMana(P.stats.MNA) * (1 + classBonus("mana"));
const shadowDmgOf = s => FORMULA.shadowDamage(SHADOWS[s.id].dmg * (1 + (s.level - 1) * 0.1) * mobRank(s.rank).sh, P.stats.SDW) * shadowMult()
  * (now() < (player.shadowBuffUntil || 0) ? 1.3 : 1);
...
function requestSkill(){
  ...
  player.skillCd = player.skillCdMax = info.cd;
...
function infRuneChance(w){ return w < 10 ? 0 : w < 20 ? .08 : w < 30 ? .18 : w < 50 ? .32 : .5; }
function infWaveClear(d){
  ...
  if (Math.random() < infRuneChance(w)){
    // las runas del Vacío y de la Eternidad solo salen aquí, y solo desde lejos
    const pool = w >= 30 ? ["Void", "Eternity", "Health", "Gems", "Cash", "Time"] : w >= 20 ? ["Void", "Health", "Gems", "Cash", "Time"] : ["Health", "Gems", "Cash", "Time"];
    const r = pool[Math.floor(Math.random() * pool.length)];
    P.runes[r] = (P.runes[r] || 0) + 1;
    banner(`RUNA: ${RUNES[r].name.toUpperCase()}`, "#5ae2ff"); note(`${RUNES[r].glyph} ${RUNES[r].name} (oleada ${w})`, "--gem");
  }
...
function clearDungeon(){
  ...
  P.dungeonsCleared++;
  const gems = Math.floor(400 * d.mode.reward * (1 + P.level*0.02) * (d.runes.Gems ? 1.1 : 1) * (1 + Math.min(10, P.runes?.Eternity || 0) * .05));
  ...
  const rune = ["Health", "Gems", "Time", "Cash"][Math.floor(Math.random()*4)];
...
function buildCharacter(cfg){
  ...
  if (cfg.hero){
    ...
    const nose = P0(3, 4, 2.5, skin); nose.position.set(0, 10 * s, 9.6 * s); neck.add(nose);
...
/* Monumentos de cada isla: tres piezas únicas por anillo que se ven de lejos
   ...
   liberan al alejarte. */
const LANDMARKS = [];
for (let i = 0; i < 13; i++) for (let k = 0; k < 3; k++){
  const a = i * 1.37 + k * 2.094 + .4;
  const r = i === 0 ? 900 + k * 120 : i * 1400 + 700 + (k - 1) * 260;
...
function playerConfig(){
  ...
  return {
    scale:1.06, skin:"#e8b98a", hero:true, iris:eyeI.id, glowEyes: glowEye, eyes: glowEye ? eyeI.id : "#12172b",
    shirt: out.shirt, sleeve: out.sleeve, pants: out.pants, boots: out.boots,
    belt:"#1a1428", buckle: out.trim,
    ...
    rig: armor === "light" ? "#0f1526" : null, rig2: out.trim,
    kneepads: armor === "light" ? "#0f1526" : null,
    coat: coat ? out.shirt : null,
    trim: coat || heavy ? out.trim : null,
    armor: coat ? null : out.plate, armorGlow: 0,
    core: plate || coat ? out.trim : null,
    pauldron: out.plate, pauldron2: plate || coat ? out.trim : null,
    spikes: heavy || (coat && P.awakened) ? out.trim : null,
    gauntlet: out.plate,
    tassets: plate ? out.plate : null,
    greaves: heavy ? out.plate : null,
    backSpikes: coat && rankIdx(P.rank) >= 5 ? out.trim : null,
    helm: null, crest: heavy ? out.trim : null,
    glove: out.trim,
    cape: capeI === "none" ? null : capeI === "shadow" ? "#120c24" : out.plate, capeLong: capeI === "long" || capeI === "shadow",
    crown: null,
    weapon: kitOf(P.weapon).color, weaponGlow: P.awakened || !!kitOf(P.weapon).glow,
...
function panelStats(){
  ...
  if (panelTab === "Look"){
    const rows = Object.keys(LOOKS).map(cat => `<div class="lookrow"><span class="lk-cat">${LOOK_CAT[cat]}</span><div class="chips">${
      LOOKS[cat].map(o => { const un = needMet(o.need), on = P.look[cat] === o.id, sw = cat === "hairColor" || cat === "eyes" || (cat === "aura" && o.id !== "none");
...
function panelItems(){
  ...
  if (panelTab === "Relics"){
    ...
    body = all.map((r, i) => { const n = P.runes[r.id] || 0; if (n) got++;
      return card({ locked:!n, tc: n ? "var(--gem)" : "var(--line)", g:r.glyph, name:r.name, sub:r.desc, tag:`×${fmt(n)}` }, i); }).join("");
  } else {
...
function panelMap(){
  ...
  if (mapTab === "Regions"){
    ...
    right = `<div class="list">${Object.entries(DUNGEON_MODES).map(([id, m]) => m.infinite ? `<div class="item inf"><span class="g">♾</span>
      <span class="meta"><b>${m.name}</b><span>Oleadas sin fin, cada una más dura · jefe cada 10 · runas desde la oleada 10 (exclusivas desde la 20 y la 30) · récord: oleada ${P.infBest || 0}</span></span>
