...
function panelPrimary(){
  const btn = modal.querySelector(".sheet [data-equip], .sheet [data-weapon]:not([disabled]), .rcard [data-ring]:not([disabled])");
...
function panelKey(k, e){
  ...
  const cards = [...modal.querySelectorAll(".cards .card[data-pick], .cards .card[data-wpick], .cards .card[data-beast]")];
  ...
  const c = cards[j];
  if (c.dataset.pick) shadowSel = c.dataset.pick; else if (c.dataset.wpick) shopSel = c.dataset.wpick; else beastSel = c.dataset.beast;
  SFX.ui(); renderPanel();
  return true;
}
...
modal.addEventListener("click", e => {
  ...
  if (t.dataset.pick){
    ...
    shadowSel = t.dataset.pick; SFX.ui(); renderPanel(); return;
  }
