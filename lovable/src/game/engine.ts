...
const ISLANDS = [
  ...
  { id:"BeastMonarch", name:"Dominio del Monarca Bestia",  level:1100, theme:"savage",  enemy:{ name:"Bestia Menor",     lvl:1112, hp:4.1e16,  dmg:5.9e13 },brute:{ name:"Bestia Mayor",     lvl:1150,hp:1.8e17,dmg:1.3e14 },boss:{ name:"Monarca de las Bestias",lvl:1190, hp:2.5e18,  dmg:3.8e14,shadow:"Antares" },  shadow:"Antares",  weapons:["MonarchClaw","BeastMaul"] },
  ...
  { id:"ShadowRealm",  name:"Trono del Rey de las Sombras",level:1400, theme:"throne",  enemy:{ name:"Soberano Caído",   lvl:1412, hp:4.2e19,  dmg:6.1e16 },brute:{ name:"General Monarca",  lvl:1450,hp:1.8e20,dmg:1.4e17 },boss:{ name:"Antares",               lvl:1490, hp:2.6e21,  dmg:3.9e17,shadow:"Ashborn" },  shadow:"Ashborn",  weapons:["ShadowDaggers","AshbornBlade"] },
];
...
function reconcile(d){
  ...
  p.tierMax = clamp(Math.round(num(p.tierMax, 0, 0)), 0, 99);
  p.tier = clamp(Math.round(num(p.tier, 0, 0)), 0, p.tierMax);
  p.tierKills = Math.round(num(p.tierKills, 0, 0));
  p.essence = Math.floor(num(p.essence, 0, 0));
  if (typeof p.seals !== "object" || !p.seals) p.seals = {};
  for (const k of ["power","vigor","legion","greed"]) p.seals[k] = clamp(Math.round(num(p.seals[k], 0, 0)), 0, 50);
  return p;
}
...
const baseDamage  = () => FORMULA.physicalDamage(weaponDmg(), P.stats.STR) * (blessed() ? 1.4 : 1) * (formActive() ? FORM.dmg : 1)
  ...
  * (1 + Math.min(10, P.runes?.Void || 0) * .05) * (1 + runeN("Monarch", 3) * .3) * (1 + (P.seals?.power || 0) * .12);
const relicBonus  = effect => Object.keys(P.relics).reduce((a, id) => a + (RELICS[id]?.effect === effect ? RELICS[id].value : 0), 0);
const shadowMult  = () => 1 + relicBonus("shadowDmg") + classBonus("shadow") + talentLv("legion") * 0.15 + (P.seals?.legion || 0) * .12;
...
function enemyDef(kind){
  ...
  return enemyDefAt(player.x, player.y, kind);
}
function enemyDefAt(px, py, kind){
  const isle = regionAt(px, py);
  ...
  const idx = ringAt(px, py);
  const next = ISLANDS[idx + 1];
  const def = { ...base };
  if (next){
    const d = Math.hypot(px - CFG.WORLD.cx, py - CFG.WORLD.cy);
...
function spawnEnemy(def, x, y, kind){
  const d = def || (x !== undefined && !dungeon ? enemyDefAt(x, y, kind) : enemyDef(kind || (Math.random() < 0.25 ? "brute" : "normal")));
...
function killEnemy(e){
  ...
  P.kills++;
  const R = mobRank(e.rank), loot = (e.boss ? 1 : R.loot) * threatLootOf(e);
  threatOnKill(e, R);
  gainXP(FORMULA.expReward(e.level, P.rebirths) * loot * (1 + (P.seals?.greed || 0) * .1));
  ...
  P.cash += cash * (1 + (P.seals?.greed || 0) * .1);
...
/* Niveles de Amenaza: al acabar las islas el mundo se quedaba fácil. Cada
   nivel multiplica la vida (×3,2) y el daño (×1,9) de todo lo que aparece
   fuera de mazmorras, añade modificadores que cambian cómo se pelea y paga
   más (×1,55) además de Esencia del Abismo para los Sellos. Para abrir el
   siguiente hay que cazar en el más alto abierto, y a partir del 2 pide
   renacer: así renacer vuelve a importar. */
const THREAT_AFFIX = [
  { id:"furia",  name:"Furiosos",     desc:"+60% de daño",                  dmg:1.6 },
  { id:"blind",  name:"Blindados",    desc:"+80% de vida",                  hp:1.8 },
  { id:"rapid",  name:"Veloces",      desc:"+35% de velocidad",             spd:1.35 },
  { id:"vamp",   name:"Vampíricos",   desc:"se curan un 4% al golpearte",   vamp:.04 },
  { id:"boom",   name:"Explosivos",   desc:"estallan al morir: aléjate",    boom:true },
  { id:"regen",  name:"Regenerantes", desc:"recuperan un 1,5% de vida por segundo", regen:.015 },
];
const threatHP   = t => Math.pow(3.2, t);
const threatDmg  = t => Math.pow(1.9, t);
const threatLoot = t => Math.pow(1.55, t);
const threatNeed = t => 200 + t * 150;                  // bajas en el nivel t para abrir t+1
const threatRebirthNeed = t => Math.max(0, t - 1);      // renaceres para abrir el nivel t
function threatAffixes(t){
  const n = t <= 0 ? 0 : t < 4 ? 1 : t < 8 ? 2 : 3, out = [];
  for (let k = 0; out.length < n; k++){ const a = THREAT_AFFIX[(t * 5 + k * 2) % THREAT_AFFIX.length]; if (!out.includes(a)) out.push(a); }
  return out;
}
const threatLootOf = e => e.threat ? threatLoot(e.threat) : 1;
function applyThreat(e){
  const t = dungeon ? 0 : P.tier;
  if (!t) return;
  e.threat = t;
  let hp = threatHP(t), dm = threatDmg(t);
  for (const a of threatAffixes(t)){
    hp *= a.hp || 1; dm *= a.dmg || 1;
    if (a.spd) e.spdMult = a.spd; if (a.vamp) e.vamp = a.vamp; if (a.boom) e.boom = true; if (a.regen) e.regen = a.regen;
  }
  e.hp = e.maxHp = e.maxHp * hp; e.dmgMult = (e.dmgMult || 1) * dm; e.level += t * 25;
}
function threatOnKill(e, R){
  if (!e.threat) return;
  const ess = Math.max(1, Math.round(e.threat * (e.boss ? 25 : Math.min(10, R.loot))));
  P.essence += ess;
  if (e.boom && Math.hypot(player.x - e.x, player.y - e.y) < 150){ hitPlayer(e.def.dmg * (e.dmgMult || 1) * 1.5); }
  if (e.boom){ ring(e.x, e.y, 150, "#ff6a3a", .6); burst(e.x, e.y, 26, "#ff8a3a", 30); }
  if (e.threat === P.tierMax){
    P.tierKills++;
    if (P.tierKills === threatNeed(P.tierMax)){
      const nx = P.tierMax + 1;
      if (P.rebirths >= threatRebirthNeed(nx)) note(`Amenaza ${nx} lista para abrir en Atributos → Abismo`, "--monarch");
      else note(`Amenaza ${nx}: te falta llegar a Renacer ${threatRebirthNeed(nx)}`, "--monarch");
    }
  }
}
function threatUnlock(){
  const nx = P.tierMax + 1;
  if (P.tierKills < threatNeed(P.tierMax) || P.rebirths < threatRebirthNeed(nx)) return;
  P.tierMax = nx; P.tierKills = 0; setThreat(nx);
  banner(`AMENAZA ${nx}`, "#ff4d61"); SFX.bossPhase?.(4);
}
function setThreat(t){
  if (dungeon) return note("Sal de la mazmorra para cambiar la amenaza", "--hp");
  t = clamp(t, 0, P.tierMax); if (t === P.tier) return;
  P.tier = t; enemies = enemies.filter(e => e.boss); spawnT = 0;
  note(t ? `Amenaza ${t}: ${threatAffixes(t).map(a => a.name).join(", ")}` : "Amenaza 0: mundo normal", "--monarch");
  save(); dirty = true; renderPanel();
}
const SEALS = {
  power:  { name:"Sello del Poder",   desc:"+12% de daño por nivel",           g:"⚔️" },
  vigor:  { name:"Sello del Vigor",   desc:"+10% de vida por nivel",           g:"❤️" },
  legion: { name:"Sello de la Legión",desc:"+12% de daño de sombras por nivel",g:"👥" },
  greed:  { name:"Sello de la Codicia",desc:"+10% de oro y EXP por nivel",     g:"💰" },
};
const sealCost = lv => ({ ess: Math.floor(25 * Math.pow(1.45, lv)), cash: 1e9 * Math.pow(40, lv) });
function buySeal(id){
  const lv = P.seals[id] || 0, c = sealCost(lv);
  if (lv >= 50 || P.essence < c.ess || P.cash < c.cash) return;
  P.essence -= c.ess; P.cash -= c.cash; P.seals[id] = lv + 1;
  player.hp = Math.min(player.hp, maxHP());
  note(`${SEALS[id].name} ${lv + 1}`, "--monarch"); SFX.levelUp?.(); save(); dirty = true; renderPanel();
}
function panelAbyss(nav){
  const tiers = Array.from({ length:P.tierMax + 1 }, (_, t) => `<button class="chip ${P.tier === t ? "sel" : ""}" data-tier="${t}">Amenaza ${t}</button>`).join("");
  const aff = threatAffixes(P.tier);
  const nx = P.tierMax + 1, need = threatNeed(P.tierMax), rbNeed = threatRebirthNeed(nx);
  const ready = P.tierKills >= need && P.rebirths >= rbNeed;
  const seals = Object.entries(SEALS).map(([id, S]) => { const lv = P.seals[id] || 0, c = sealCost(lv), ok = lv < 50 && P.essence >= c.ess && P.cash >= c.cash;
    return `<div class="item"><span class="g">${S.g}</span><span class="meta"><b>${S.name} · Nv ${lv}/50</b><span>${S.desc} · coste ${fmt(c.ess)} esencia + ${fmt(c.cash)} oro</span></span>
      <button class="btn violet" data-seal="${id}" ${ok ? "" : "disabled"}>Mejorar</button></div>`; }).join("");
  return shell("Abismo", nav + `<div class="spinbox">
      <div class="spinhead"><div><small>Amenaza activa</small><b style="color:#ff6a7a">${P.tier}</b></div>
        <div class="spins"><small>Esencia del Abismo</small><b>${fmt(P.essence)}</b></div></div>
      <div class="chips">${tiers}</div>
      <p class="hint">${P.tier ? `Enemigos con ×${fmt(threatHP(P.tier))} vida y ×${threatDmg(P.tier).toFixed(1)} daño · botín ×${threatLoot(P.tier).toFixed(2)} · ${aff.map(a => `<b>${a.name}</b> (${a.desc})`).join(" · ")}`
        : "Mundo normal. Sube la amenaza para que los enemigos sean más duros y den más botín y Esencia."}</p>
      <div class="collect"><span>Para abrir Amenaza ${nx}: bajas en Amenaza ${P.tierMax}${rbNeed ? ` y Renacer ${rbNeed}` : ""}</span>
        <div class="sbar xp"><i style="width:${Math.min(100, P.tierKills / need * 100)}%"></i><em>${fmt(Math.min(P.tierKills, need))} / ${fmt(need)}</em></div></div>
      <button class="btn gold big-w" id="tier-btn" ${ready ? "" : "disabled"}>${ready ? `Abrir Amenaza ${nx}` : P.rebirths < rbNeed ? `Necesitas Renacer ${rbNeed}` : "Sigue cazando"}</button></div>
    <h3 class="subh">Sellos del Abismo</h3><div class="list">${seals}</div>
    <p class="kbhelp">La Esencia solo cae con amenaza 1 o más. Los modificadores cambian con cada nivel, así que cada amenaza se juega distinto.</p>`);
}
/* Renacer: el primero a nivel 100 y luego uno cada 50 niveles más (150, 200,
   250… hasta 1000). El coste en oro crece ×2,2 por renacer. */
const rebirthLevel = () => Math.min(FLAGS.LEVEL_CAP, 100 + P.rebirths * 50);
const rebirthCost  = () => Math.round(2e5 * Math.pow(6, P.rebirths));
...
function update(dt){
  ...
  if (dungeon && !dungeon.mode.infinite){
    ...
    const camp = P.tutorial >= 2 ? nearestCamp(player.x, player.y, 1500) : null;
    for (let i=0;i<batch;i++){
      if (camp){
        const a = rnd(0, Math.PI * 2), r = rnd(0, 220), isle = ISLANDS[camp.ring];
        const kind = camp.brute ? (Math.random() < .6 ? "brute" : "normal") : (Math.random() < .12 ? "brute" : "normal");
        spawnEnemy(null, camp.x + Math.cos(a) * r, camp.y + Math.sin(a) * r, kind);
      } else spawnEnemy();
    }
  ...
  for (const e of enemies){
    ...
    if (d > stop){
      const sp = (98 + Math.min(60, e.level * 0.05)) * (e.boss ? 0.8 + e.phase*0.12 : 1) * (e.slowT > 0 ? .5 : 1) * (e.spdMult || 1);
    ...
    if (e.telegraph){
      ...
      if (e.boss){
        ...
        e.atkCd = 1.35;
        if (d <= stop + 26 && player.h < 50){ hitPlayer(e.def.dmg * (e.dmgMult || 1)); if (e.vamp) e.hp = Math.min(e.maxHp, e.hp + e.maxHp * e.vamp); }
      }
    }
  }
  separate(dt);
  ...
  for (const p of parts){
    ...
    if (!p.ghost){
      ...
      p.vx *= 0.94; p.vy *= 0.94; p.vh -= 620 * (p.g ?? 1) * dt;
...
function buildCharacter(cfg){
  ...
  if (cfg.weapon){
    ...
    if (kind === "axe"){
      const haft = P0(4.5, 44, 4.5, "#5a3c22"); haft.position.y = 20 * s; wg.add(haft);
      for (const y of [4, 9, 14]){ const wrap = P0(5.4, 2, 5.4, "#2a1a10"); wrap.position.y = y * s; wg.add(wrap); }
      const pom = P0(6.5, 4, 6.5, "#3a3a44"); pom.position.y = -2 * s; wg.add(pom);
      const collar = P0(6.5, 9, 6.5, "#3a3a44"); collar.position.y = 38 * s; wg.add(collar);
      const head2 = P0(15, 17, 3.4, cfg.weapon); head2.position.set(-9*s, 38*s, 0); wg.add(head2);
      const beard = P0(6, 9, 3, cfg.weapon); beard.position.set(-15*s, 30*s, 0); beard.rotation.z = .5; wg.add(beard);
      const bit = P0(2.4, 22, 3.6, "#f4f6fa", { ...opts, emissive:"#9aa8c0", emissiveIntensity:.35 }); bit.position.set(-17*s, 37*s, 0); wg.add(bit);
      const back = P0(8, 6, 3, "#3a3a44"); back.position.set(6*s, 39*s, 0); back.rotation.z = -.4; wg.add(back);
      const top = P0(3, 8, 3, "#3a3a44"); top.position.y = 46 * s; wg.add(top);
    } else if (kind === "scythe"){
      const haft = P0(4, 54, 4, "#2e2420"); haft.position.y = 25 * s; wg.add(haft);
      for (const y of [8, 30]){ const band = P0(5.2, 2.4, 5.2, "#8a7a5a"); band.position.y = y * s; wg.add(band); }
      const sock = P0(7, 7, 7, "#1a1418"); sock.position.y = 50 * s; wg.add(sock);
      const segs = [[6, 51, 10, .12], [14, 50, 10, -.18], [21, 46, 9, -.55], [26, 40, 8, -.95]];
      for (const [x, y, w, rz] of segs){
        const b = P0(w, 4.5, 2.6, cfg.weapon); b.position.set(x*s, y*s, 0); b.rotation.z = rz; wg.add(b);
        const e = P0(w, 1.4, 2.8, "#f2f4fa", { ...opts, emissive:"#a8b4d0", emissiveIntensity:.35 });
        e.position.set(x*s, (y - 2.6)*s, 0); e.rotation.z = rz; wg.add(e); }
    } else if (kind === "staff"){
      const haft = P0(4, 50, 4, "#5a4a30"); haft.position.y = 25 * s; wg.add(haft);
      for (const y of [6, 20, 34]){ const band = P0(5, 2, 5, "#c9a33a"); band.position.y = y * s; wg.add(band); }
      const foot = P0(5, 5, 5, "#c9a33a"); foot.position.y = -1 * s; wg.add(foot);
      const orb = new THREE.Mesh(new THREE.SphereGeometry(6.5*s, 14, 12),
        mat(cfg.weapon, { ...opts, emissive: cfg.weapon, emissiveIntensity:1.1 }));
      orb.position.y = 56 * s; wg.add(orb);
      for (let i = 0; i < 3; i++){ const a = i / 3 * Math.PI * 2;
        const prong = P0(2, 13, 2, "#c9a33a"); prong.position.set(Math.cos(a) * 6 * s, 54 * s, Math.sin(a) * 6 * s);
        prong.rotation.set(Math.sin(a) * .35, 0, -Math.cos(a) * .35); wg.add(prong); }
      const halo = new THREE.Sprite(new THREE.SpriteMaterial({ map: glowTexture(cfg.weapon), transparent:true, depthWrite:false, blending:THREE.AdditiveBlending, opacity:.6 }));
      halo.scale.set(34 * s, 34 * s, 1); halo.position.y = 56 * s; wg.add(halo);
    } else if (kind === "polearm"){
      const haft = P0(4, 50, 4, "#3a2a22"); haft.position.y = 23 * s; wg.add(haft);
      for (const y of [4, 16, 40]){ const band = P0(5, 2, 5, "#8a7a5a"); band.position.y = y * s; wg.add(band); }
      const collar = P0(6, 5, 6, "#2a2a30"); collar.position.y = 47 * s; wg.add(collar);
      const tip = P0(6.5, 18, 2.6, cfg.weapon); tip.position.y = 58 * s; wg.add(tip);
      const pt = P0(4, 6, 2.4, cfg.weapon); pt.position.y = 69 * s; pt.rotation.z = .78; wg.add(pt);
      const edge = P0(1.4, 18, 2.8, "#f2f4fa", { ...opts, emissive:"#a8b4d0", emissiveIntensity:.35 }); edge.position.set(3.3*s, 58*s, 0); wg.add(edge);
      const tassel = P0(3, 8, 3, "#b02a2a"); tassel.position.set(0, 43 * s, 0); wg.add(tassel);
    } else if (kind === "dagger"){
      ...
      // hoja curva en tres tramos con filo claro y hamon
      for (const [y, h, z, rx] of [[18, 16, 0, 0], [33, 16, .9, .06], [47, 14, 2.4, .14]]){
        const b = P0(3, h, 1.4, cfg.weapon, { ...opts, emissive:cfg.weapon, emissiveIntensity:.25 });
        b.position.set(0, y * s, z * s); b.rotation.x = rx; wg.add(b);
        const ed = P0(1, h, 1.6, "#ffffff", { ...opts, emissive:"#cfe0ff", emissiveIntensity:.5 });
        ed.position.set(1.6 * s, y * s, z * s); ed.rotation.x = rx; wg.add(ed); }
      const kis = P0(2.4, 5, 1.4, "#ffffff"); kis.position.set(.4 * s, 56 * s, 3.6 * s); kis.rotation.set(.2, 0, -.5); wg.add(kis);
      const habaki = P0(4, 3, 2.4, "#c9a33a"); habaki.position.y = 10.5 * s; wg.add(habaki);
      ...
      const tipS = P0(4.4, 7, 2.6, cfg.weapon, bopts); tipS.position.y = 50 * s; tipS.rotation.z = .78; tipS.scale.y = .8; wg.add(tipS);
      const fuller = P0(1.6, 28, 3.4, glowing ? "#ffffff" : "#6a7488", glowing ? { ...opts, emissive:cfg.weapon, emissiveIntensity:2 } : opts);
      fuller.position.y = 28 * s; wg.add(fuller);
      const edgeL = P0(1.4, 40, 1.4, glowing ? "#ffffff" : "#f4f6fa", { ...opts, emissive: glowing ? cfg.weapon : "#8a96b0", emissiveIntensity: glowing ? 1.8 : .25 });
      edgeL.position.set(-2.6 * s, 31 * s, 0); wg.add(edgeL);
      const guard = P0(16, 4, 7, glowing ? "#1b1230" : "#8a7a5a"); guard.position.y = 11 * s; wg.add(guard);
      for (const sd of [-1, 1]){ const q = P0(4, 5, 5, glowing ? cfg.weapon : "#c9a33a", glowing ? { ...opts, emissive:cfg.weapon, emissiveIntensity:1 } : opts);
        q.position.set(sd * 9 * s, 12.5 * s, 0); wg.add(q); }
      const gem = P0(3, 3, 7.6, glowing ? "#ffffff" : "#c0392b", { ...opts, emissive: glowing ? cfg.weapon : "#c0392b", emissiveIntensity:.8 });
      gem.position.y = 11 * s; wg.add(gem);
      const grip = P0(5, 11, 5, glowing ? "#0e0a1c" : "#4a3a2a"); grip.position.y = 3 * s; wg.add(grip);
      for (const y of [0, 3, 6]){ const wr = P0(5.6, 1.2, 5.6, glowing ? "#2a1e44" : "#2a1a10"); wr.position.y = y * s; wg.add(wr); }
      const pom = P0(6, 5, 6, glowing ? "#1b1230" : "#8a7a5a"); pom.position.y = -4.5 * s; wg.add(pom);
      if (glowing){
    ...
    g.weapon = wg;
    // postura de carga: las pesadas descansan sobre el hombro, las hojas en
    // guardia hacia delante y las astas de pie como un bastón
    g.carry = WEAPON_CARRY[kind] || WEAPON_CARRY.sword;
    g.userData.slashCol = cfg.weaponGlow || kind === "monarchblade" ? cfg.weapon : "#dfe8ff";
    g.weaponTip = new THREE.Object3D();
    g.weaponTip.position.y = (WEAPON_REACH[kind] || 40) * s; wg.add(g.weaponTip);
  }
  addFeatures(detail, cfg, s, opts);
  g.scaleRef = s;
  g.setLod = far => {
...
const WEAPON_CARRY = {
  monarchblade:{ arm:-.55, x:-.05, z:-.2, sway:.14 },
  greatsword:  { arm:-.55, x:-.05, z:-.2, sway:.14 },
  hammer:      { arm:-.5,  x:-.1,  z:-.2, sway:.14 },
  axe:         { arm:-.5,  x:-.1,  z:-.2, sway:.16 },
  scythe:      { arm:-.45, x:-.05, z:-.25, sway:.14 },
  polearm:     { arm:-.3,  x:.28,  z:.05, sway:.2 },
  spear:       { arm:-.3,  x:.28,  z:.05, sway:.2 },
  staff:       { arm:-.25, x:.22,  z:.04, sway:.22 },
  sword:       { arm:-.3,  x:1.25, z:-.1, sway:.3 },
  katana:      { arm:-.3,  x:1.3,  z:-.12,sway:.3 },
  dagger:      { arm:-.2,  x:1.9,  z:0,   sway:.4 },
  claw:        { arm:-.15, x:.2,   z:0,   sway:.45 },
  orb:         { arm:-.35, x:0,    z:0,   sway:.35 },
};
const WEAPON_REACH = { monarchblade:82, greatsword:64, hammer:44, axe:44, scythe:50, polearm:56, spear:72,
  staff:56, sword:48, katana:52, dagger:32, claw:22, orb:16 };
function poseCharacter(v, o){
  ...
  if (o.swing > 0){
    ...
    const C = v.carry;
    v.arms[1].rotation.x = C.arm + (dash ? .5 : 0) + walk * .55 * C.sway * mv - breathe * .03;
    v.arms[1].rotation.z = armOpen + .08;
  } else {
    v.arms[1].rotation.x = air ? -1.1 : dash ? .9 : walk * .55 * mv + (mv ? 0 : -breathe * .04);
  }
  // el arma pasa de la postura de carga a alinearse con el brazo al golpear
  if (v.weapon && v.carry){
    const fight = o.swing > 0 || atk > 0, C = v.carry, k = fight ? .5 : .14;
    const tx = fight ? 0 : C.x + (mv ? Math.sin(o.step) * .05 : breathe * .02), tz = fight ? 0 : C.z;
    v.weapon.rotation.x += (tx - v.weapon.rotation.x) * k;
    v.weapon.rotation.z += (tz - v.weapon.rotation.z) * k;
  }
...
const THEMES = {
  ...
  savage:  { g1:"#5a3a26", g2:"#4e3220", sky1:"#5a1a0e", sky2:"#e8904a", fog:"#8a4a28", amp:42, sun:"#ffc890", prop:"savage" },
  throne:  { g1:"#141026", g2:"#100c1f", sky1:"#05020f", sky2:"#3a0f5a", fog:"#1a0a2e", amp:14, sun:"#c9a0ff", prop:"throne" },
...
const MUSIC_SCALES = {
  ...
  savage:  { tempo:1.8, acordes:[[73,110,147],[65,98,131],[82,123,165],[73,110,147]],     melodia:[294,330,392,440] },
...
function borderDist(x, z){
  if (dungeon) return 1e9;
  const r = Math.hypot(x - CFG.WORLD.cx, z - CFG.WORLD.cy), RW = CFG.RING_WIDTH;
  const k = Math.round(r / RW);
  return k < 1 || k > ISLANDS.length - 1 ? 1e9 : Math.abs(r - k * RW);
}
function terrainH(x, z){
  const a = themeNow().amp;
  const base = a <= 0.5 ? 0 : (Math.sin(x*0.0021) * Math.cos(z*0.0017) * .65 + Math.sin((x+z)*0.0009) * .35) * a;
  const bd = borderDist(x, z);
  if (bd >= CHANNEL * 2) return base;
  const k = clamp((bd - CHANNEL * .55) / (CHANNEL * 1.45), 0, 1), s = k * k * (3 - 2 * k);
  return -16 + (base + 16) * s;
}
/* Ambiente de las islas avanzadas. En la última, el Trono del Rey de las
   Sombras: una corona gigante gira en el cielo, columnas de sombra se alzan
   en el horizonte, cae ceniza violeta y los relámpagos iluminan el suelo. */
const REALM = { sky:null, bolt:null, flash:0, nextBolt:4, hemi0:null };
function syncRealmFx(dt){
  const ring = dungeon ? -1 : ringAt(player.x, player.y), th = dungeon ? null : ISLANDS[ring].theme;
  const throne = th === "throne";
  if (throne && !REALM.sky){
    const g = new THREE.Group();
    const V = new THREE.Color("#9a4dff"), L = new THREE.Color("#e0c0ff");
    const mk = c => new THREE.MeshBasicMaterial({ color:c, fog:false, transparent:true, opacity:.95 });
    const crown = new THREE.Group();
    crown.add(new THREE.Mesh(new THREE.TorusGeometry(520, 34, 10, 64), mk(V)));
    crown.add(new THREE.Mesh(new THREE.TorusGeometry(470, 12, 8, 64), mk(L)));
    for (let i = 0; i < 12; i++){
      const a = i / 12 * Math.PI * 2, sp = new THREE.Mesh(new THREE.ConeGeometry(46, i % 2 ? 180 : 300, 6), mk(i % 2 ? L : V));
      sp.position.set(Math.cos(a) * 520, Math.sin(a) * 520, 0); sp.rotation.z = a - Math.PI / 2; crown.add(sp);
    }
    crown.rotation.x = Math.PI / 2.4; crown.position.y = 1700; g.add(crown); g.crown = crown;
    const halo = new THREE.Sprite(new THREE.SpriteMaterial({ map:glowTexture("#9a4dff"), fog:false, transparent:true, depthWrite:false, blending:THREE.AdditiveBlending, opacity:.6 }));
    halo.scale.set(2600, 2600, 1); halo.position.y = 1700; g.add(halo); g.halo = halo;
    // columnas de sombra en el horizonte
    g.pillars = [];
    for (let i = 0; i < 8; i++){
      const a = i / 8 * Math.PI * 2 + .3, col = new THREE.Group();
      const body = new THREE.Mesh(new THREE.CylinderGeometry(70, 150, 2600, 8, 1, true), new THREE.MeshBasicMaterial({ color:0x07040f, fog:false, transparent:true, opacity:.9, side:THREE.DoubleSide }));
      body.position.y = 1300; col.add(body);
      const core = new THREE.Mesh(new THREE.CylinderGeometry(22, 40, 2600, 6, 1, true), new THREE.MeshBasicMaterial({ color:V, fog:false, transparent:true, opacity:.55,
        blending:THREE.AdditiveBlending, depthWrite:false, side:THREE.DoubleSide }));
      core.position.y = 1300; col.add(core);
      col.position.set(Math.cos(a) * 3400, 0, Math.sin(a) * 3400); g.add(col); g.pillars.push(col);
    }
    const bolt = new THREE.Mesh(new THREE.BoxGeometry(14, 1800, 14), new THREE.MeshBasicMaterial({ color:0xe0c0ff, fog:false, transparent:true, opacity:0,
      blending:THREE.AdditiveBlending, depthWrite:false }));
    bolt.position.y = 900; g.add(bolt); REALM.bolt = bolt;
    scene.add(g); REALM.sky = g;
  }
  if (REALM.sky){
    REALM.sky.visible = throne;
    if (throne){
      const t = now();
      REALM.sky.position.set(player.x, 0, player.y);
      REALM.sky.crown.rotation.z = t * .08;
      REALM.sky.halo.material.opacity = .45 + Math.sin(t * .9) * .15;
      REALM.sky.pillars.forEach((c, i) => { c.children[1].material.opacity = .4 + Math.sin(t * 1.6 + i) * .2; });
      // relámpagos
      if (REALM.hemi0 === null) REALM.hemi0 = hemiLight.intensity;
      REALM.nextBolt -= dt;
      if (REALM.nextBolt <= 0){
        REALM.nextBolt = rnd(3.5, 8); REALM.flash = .35;
        REALM.bolt.position.x = rnd(-900, 900); REALM.bolt.position.z = rnd(-900, 900);
        REALM.bolt.rotation.z = rnd(-.15, .15);
        SFX.bossHit?.("slam");
      }
      REALM.flash = Math.max(0, REALM.flash - dt);
      REALM.bolt.material.opacity = REALM.flash > 0 ? .9 * (REALM.flash / .35) : 0;
      hemiLight.intensity = REALM.hemi0 * (1 + REALM.flash * 4);
      // ceniza violeta que cae
      if (Math.random() < .6)
        parts.push({ x:player.x + rnd(-700, 700), y:player.y + rnd(-700, 700), h:rnd(220, 420), vx:rnd(-10, 10), vy:rnd(-10, 10), vh:-rnd(40, 80),
                     life:rnd(3, 5), color: Math.random() < .5 ? "#c9a0ff" : "#2a1a40", size:rnd(3, 6), g:0 });
    } else if (REALM.hemi0 !== null){ hemiLight.intensity = REALM.hemi0; }
  }
  // resto de islas avanzadas: brasas o chispas de su color flotando en el aire
  if (!throne && ring >= 8 && Math.random() < .5){
    const col = (THEMES[th] || THEMES.city).sun;
    parts.push({ x:player.x + rnd(-600, 600), y:player.y + rnd(-600, 600), h:rnd(0, 40), vx:rnd(-6, 6), vy:rnd(-6, 6), vh:rnd(20, 50),
                 life:rnd(2, 3.5), color:col, size:rnd(2, 4), g:0 });
  }
}
let waterRings = null;
function syncWater(dt){
  if (!waterRings){
    waterRings = new THREE.Group();
    const RW = CFG.RING_WIDTH;
    const wm = new THREE.MeshPhongMaterial({ color:0x2a9fe0, emissive:0x0a3a66, shininess:90, specular:0xbfe8ff,
      transparent:true, opacity:.82, depthWrite:false });
    const foam = new THREE.MeshBasicMaterial({ color:0xeaf8ff, transparent:true, opacity:.7, depthWrite:false });
    const sand = new THREE.MeshLambertMaterial({ color:0xe8d49a, transparent:true, opacity:.9, depthWrite:false });
    for (let k = 1; k < ISLANDS.length; k++){
      const R0 = k * RW, seg = 720;
      for (const [inner, outer, m, y] of [[R0 - CHANNEL * 1.25, R0 + CHANNEL * 1.25, sand, -.5],
                                          [R0 - CHANNEL * .8, R0 + CHANNEL * .8, wm, -4],
                                          [R0 - CHANNEL * .82, R0 - CHANNEL * .74, foam, -3.5],
                                          [R0 + CHANNEL * .74, R0 + CHANNEL * .82, foam, -3.5]]){
        const ring = new THREE.Mesh(new THREE.RingGeometry(inner, outer, seg, 1), m);
        ring.rotation.x = -Math.PI / 2; ring.position.set(CFG.WORLD.cx, y, CFG.WORLD.cy);
        ring.renderOrder = m === sand ? 1 : m === wm ? 2 : 3;
        waterRings.add(ring);
      }
    }
    waterRings.userData.wm = wm; waterRings.userData.foam = foam;
    scene.add(waterRings);
  }
  waterRings.visible = !dungeon;
  const t = now();
  waterRings.userData.wm.emissive.setRGB(.04, .2 + Math.sin(t * 1.3) * .03, .36 + Math.sin(t * 1.3) * .04);
  waterRings.userData.foam.opacity = .45 + Math.sin(t * 2.2) * .25;
}
...
function groundDetail(x, th){
  ...
  if (kind === "forest" || kind === "royal" || kind === "shrine" || kind === "city" || kind === "guild"){
    ...
    // losas de obsidiana con grietas de energía y círculos de runas
    x.globalAlpha = .5; x.strokeStyle = "#05030c"; x.lineWidth = 2;
    for (let i = 0; i <= 256; i += 64){ x.beginPath(); x.moveTo(i, 0); x.lineTo(i, 256); x.moveTo(0, i); x.lineTo(256, i); x.stroke(); }
    cracks(7, "#8a3aff", 2.2, .8); cracks(7, "#e0c0ff", .8, .7);
    x.globalAlpha = .45; x.strokeStyle = "#b07cff"; x.lineWidth = 1.2;
    for (let i = 0; i < 2; i++){ const cx2 = R()*256, cy2 = R()*256, rr = 14 + R()*12;
      x.beginPath(); x.arc(cx2, cy2, rr, 0, 6.3); x.stroke(); x.beginPath(); x.arc(cx2, cy2, rr * .6, 0, 6.3); x.stroke(); }
    blobs(10, "#c9a0ff", .8, 1.6, .9);
  } else if (kind === "savage"){
    blobs(24, "#3a2416", 8, 26, .3);
    tufts(50, "#6a7a2a", .7);
    blobs(8, "#e8dcc0", 1.5, 3, .7);          // huesos sueltos
    cracks(4, "#2a180e", 1.6, .5);
  } else if (kind === "urban"){
...
function buildProp(kind, h){
  ...
  switch (kind){
    ...
    case "throne": {
      const V = "#9a4dff", gV = glowOpts(V);
      if (roll < 3){                                   // aguja de obsidiana con vetas
        add(46, 150 + v, 46, "#0c0918", 0, 0, 0);
        add(30, 70, 30, "#120d24", 0, 150 + v, 0);
        for (const [x2, y2] of [[23, 40], [-23, 90], [23, 130]]) add(3, 40, 3, V, x2, y2, 0, gV);
        add(10, 20, 10, V, 0, 220 + v, 0, gV);
        break;
      }
      if (roll === 3){                                 // brasero de fuego de sombra
        add(40, 36, 40, "#1a1430", 0, 0, 0);
        add(54, 8, 54, "#2a2046", 0, 36, 0);
        add(34, 30, 34, V, 0, 44, 0, gV);
        add(18, 30, 18, "#e0c0ff", 0, 60, 0, glowOpts("#e0c0ff"));
        break;
      }
      if (roll === 4){                                 // columna rota del trono
        add(50, 12, 50, "#1a1430", 0, 0, 0);
        add(36, 90 + v, 36, "#241c3c", 0, 12, 0);
        add(40, 20, 40, "#1a1430", 6, 104 + v, 4).rotation.z = .3;
        add(38, 3, 38, V, 0, 60, 0, gV);
        break;
      }
      if (roll === 5){                                 // cristal flotante
        add(10, 16, 10, "#0c0918", 0, 0, 0);
        const cr = add(28, 56, 28, V, 0, 60, 0, gV); cr.rotation.set(.6, .8, 0);
        break;
      }
      add(120, 16, 80, "#0c0918", 0, 0, 0);            // escalinata con estatua arrodillada
      add(90, 16, 60, "#140f26", 0, 16, 0);
      add(26, 50, 30, "#1a1430", 0, 32, 0);
      add(22, 22, 22, "#1a1430", 0, 82, 4);
      add(4, 4, 2, V, -5, 92, 16, gV); add(4, 4, 2, V, 5, 92, 16, gV);
      break;
    }
    case "savage": {
      if (roll < 3){                                   // árbol de la sabana
        add(18, 110 + v, 18, "#4a2e1a", 0, 0, 0);
        add(130, 22, 110, "#7a8a2a", 0, 110 + v, 0);
        add(90, 16, 80, "#8a9a34", 10, 130 + v, -6);
        break;
      }
      if (roll === 3){                                 // costillar gigante
        for (let i = 0; i < 5; i++){ const r2 = add(10, 90 - Math.abs(i - 2) * 12, 10, "#e8dcc0", -60 + i * 30, 0, 0); r2.rotation.x = .35; }
        add(150, 12, 14, "#d8ccb0", 0, 4, -20);
        break;
      }
      if (roll === 4){                                 // tótem con cara de bestia
        add(34, 150 + v, 34, "#5a3a22", 0, 0, 0);
        for (const y of [30, 80, 130]){ add(40, 20, 40, "#8a4a2a", 0, y, 0); add(8, 6, 2, "#ffb45a", -9, y + 12, 20, glowOpts("#ffb45a")); add(8, 6, 2, "#ffb45a", 9, y + 12, 20, glowOpts("#ffb45a")); }
        add(70, 14, 14, "#e8dcc0", 0, 150 + v, 0);
        break;
      }
      add(90, 50 + v / 2, 70, "#6a4a32", 0, 0, 0);     // roca roja
      add(60, 30, 50, "#7a5a3a", 8, 50 + v / 2, 4);
      break;
    }
    case "cyber": {
...
/* Zonas de caza: cinco campamentos fijos por región donde aparecen sus
   enemigos, marcados con un círculo en el suelo y un cartel con nombre y nivel. */
const CAMPS = [];
for (let i = 0; i < 13; i++) for (let k = 0; k < 5; k++){
  const h = Math.sin((i + 3) * 57.3 + k * 29.1) * 24634.6345, fr = h - Math.floor(h);
  const a = k * (Math.PI * 2 / 5) + fr * .9 + i * 1.3;
  const r = i * CFG.RING_WIDTH + CFG.RING_WIDTH * (.3 + fr * .45) + (i === 0 ? 700 : 0);
  CAMPS.push({ ring:i, k, brute: k === 4, x:CFG.WORLD.cx + Math.cos(a) * r, y:CFG.WORLD.cy + Math.sin(a) * r, view:null });
}
function nearestCamp(x, y, max){
  let best = null, bd = max;
  for (const c of CAMPS){ const d = Math.hypot(c.x - x, c.y - y); if (d < bd){ bd = d; best = c; } }
  return best;
}
function syncCamps(dt){
  const t = now();
  for (const c of CAMPS){
    const d = Math.hypot(c.x - player.x, c.y - player.y);
    if (dungeon || d > 2600){ if (c.view){ scene.remove(c.view); disposeView(c.view); c.view = null; } continue; }
    if (!c.view){
      const isle = ISLANDS[c.ring], def = enemyDefAt(c.x, c.y, c.brute ? "brute" : "normal"), col = (THEMES[isle.theme] || THEMES.city).sun;
      const g = new THREE.Group();
      const ring = new THREE.Mesh(new THREE.RingGeometry(250, 262, 64), new THREE.MeshBasicMaterial({ color:new THREE.Color(c.brute ? "#ff5d6c" : "#ffd24a"),
        transparent:true, opacity:.55, depthWrite:false, side:THREE.DoubleSide }));
      ring.rotation.x = -Math.PI / 2; ring.position.y = 3; g.add(ring); g.ring = ring;
      // cartel de madera con el nombre y nivel de la zona
      const post = part(6, 70, 6, "#5a3c22"); post.position.set(0, 35, 0);
      const board = part(70, 26, 5, "#7a5230"); board.position.set(0, 74, 0);
      const trim = part(74, 3, 6, "#c9a33a"); trim.position.set(0, 88, 0);
      const sign = new THREE.Group(); sign.add(post, board, trim); sign.position.set(0, 0, -270); g.add(sign);
      const tag = labelSprite(`${c.brute ? "☠ " : "⚔ "}${def.name} · Nv ${def.lvl}`, c.brute ? "#ff9aa4" : "#ffe9a8", 40);
      tag.scale.set(220, 55, 1); tag.position.set(0, 120, -270); g.add(tag);
      for (let i = 0; i < 6; i++){                     // antorchas alrededor
        const a = i / 6 * Math.PI * 2, tc = part(4, 26, 4, "#3a2a1a"); tc.position.set(Math.cos(a) * 256, 13, Math.sin(a) * 256); g.add(tc);
        const fl = new THREE.Sprite(new THREE.SpriteMaterial({ map:glowTexture(c.brute ? "#ff5d6c" : col), transparent:true, depthWrite:false, blending:THREE.AdditiveBlending }));
        fl.scale.set(30, 30, 1); fl.position.set(Math.cos(a) * 256, 30, Math.sin(a) * 256); g.add(fl);
      }
      g.position.set(c.x, terrainH(c.x, c.y), c.y);
      scene.add(g); c.view = g;
    }
    c.view.ring.material.opacity = .4 + Math.sin(t * 2 + c.k) * .15;
  }
}
...
const MOB_LOOK = {
  ...
  BeastMonarch:{ body:"wolf",     skin:"#5a3a26", dark:"#2a1a10", glow:"#ffb45a", hair:"#1a0e08", features:["mane", "tail", "crystals"] },
  Architect:   { body:"wraith",   skin:"#4a4a86", dark:"#1f1f44", glow:"#9fa8ff",
                 features:["halo", "crystals"] },
  ShadowRealm: { body:"humanoid", skin:"#1a1030", dark:"#0a0616", glow:"#9a4dff",
                 features:["wings", "halo", "extraEyes"],
                 kit:{ coat:"#0a0616", trim:"#7a2ae0", horns:true, hornColor:"#9a4dff", spikes:"#7a2ae0", backSpikes:"#9a4dff",
                       crown:"#e0c0ff", cape:"#1a0a33", pauldron2:"#9a4dff",
                       weapon:"#b07cff", weaponGlow:true, weaponKind:"greatsword", armor:null, helm:null } },
};
...
function labelSprite(text, color, size){
  ...
  const x = cv.getContext("2d");
  let fs = size || 56;
  x.font = `600 ${fs}px Fredoka, sans-serif`;
  while (fs > 18 && x.measureText(text).width > 488){ fs -= 2; x.font = `600 ${fs}px Fredoka, sans-serif`; }
  x.textAlign = "center"; x.textBaseline = "middle";
...
function healthBar(){
  ...
  // sin depthTest el orden lo decide renderOrder: antes la estela clara se
  // pintaba encima de la vida y todas las barras se veían blancas
  bg.renderOrder = 20; chip.renderOrder = 21; fill.renderOrder = 22;
  g.add(bg, chip, fill);
...
/* Estela del tajo: destellos aditivos que siguen la punta del arma mientras
   corta y se apagan en unas décimas. Se reutilizan siempre los mismos. */
const SLASH = { pool:[], i:0, col:null };
const _tipV = new THREE.Vector3();
function updateSlashTrail(pv, cutting, dt){
  if (!pv.weaponTip) return;
  const col = pv.userData.slashCol || "#ffffff";
  if (SLASH.col !== col || SLASH.scene !== scene){
    for (const sp of SLASH.pool){ sp.parent?.remove(sp); sp.material.dispose(); }
    SLASH.pool = []; SLASH.col = col; SLASH.scene = scene;
    for (let k = 0; k < 18; k++){
      const sp = new THREE.Sprite(new THREE.SpriteMaterial({ map: glowTexture(col), transparent:true,
        depthWrite:false, blending:THREE.AdditiveBlending, opacity:0 }));
      sp.visible = false; scene.add(sp); SLASH.pool.push(sp);
    }
  }
  if (cutting){
    pv.weaponTip.getWorldPosition(_tipV);
    const sp = SLASH.pool[SLASH.i = (SLASH.i + 1) % SLASH.pool.length];
    sp.position.copy(_tipV); sp.visible = true; sp.material.opacity = .9; sp.scale.set(34, 34, 1);
  }
  for (const sp of SLASH.pool) if (sp.visible){
    sp.material.opacity -= dt * 3.2; sp.scale.multiplyScalar(1 - dt * 1.5);
    if (sp.material.opacity <= 0) sp.visible = false;
  }
}
...
function render(dt){
  ...
  syncBorder(dt);
  syncLandmarks(dt);
  syncChests(dt); syncWater(dt); syncCamps(dt); syncRealmFx(dt);
  updateCamera(dt);
  ...
  updateSlashTrail(pv, playerSwing() > .3 && playerSwing() < .8, dt);
  ...
  for (const e of enemies){
    ...
    // islas avanzadas (anillo 8+): llama de su color detrás y brasas que suben
    const ringE = ISLANDS.findIndex(is => is.id === e.isle);
    if (ringE >= 8 && !v.userData.advFx){
      const col = (MOB_LOOK[e.isle] || {}).glow || "#ffffff", sc = v.scaleRef || 1;
      const fl = new THREE.Sprite(new THREE.SpriteMaterial({ map: glowTexture(col), transparent:true, depthWrite:false, blending:THREE.AdditiveBlending, opacity:.5 }));
      fl.scale.set(80 * sc, 120 * sc, 1); fl.position.y = 42 * sc; v.add(fl); v.userData.advFx = fl; v.userData.advCol = col;
    }
    if (v.userData.advFx){
      v.userData.advFx.material.opacity = .35 + Math.sin(now() * 5 + e.x) * .15;
      if (Math.random() < .12 && Math.hypot(e.x - player.x, e.y - player.y) < 700){
        const sc = v.scaleRef || 1;
        parts.push({ x:e.x + rnd(-16, 16) * sc, y:e.y + rnd(-16, 16) * sc, h:rnd(20, 70) * sc, vx:0, vy:0, vh:rnd(30, 70),
                     life:rnd(.4, .9), color:v.userData.advCol, size:rnd(3, 6), g:0 });
      }
    }
    if (v.userData.aura){
    ...
    if (!v.hud){
      ...
      nameSp.position.y = 15; nameSp.renderOrder = 23;
    ...
    // como en los juegos de Roblox: nombre, nivel y vida siempre a la vista de cerca
    v.hud.visible = (e.boss || e === target || dp < 650) && e.hp > 0;
    v.hud.name.visible = true;
  ...
  for (const sh of shadows){
    ...
    if (dS < 800 && Math.random() < (sh.state !== "IDLE_FOLLOW" ? .5 : .22)){
      ...
      parts.push({ x:sh.x + rnd(-14, 14) * sc, y:sh.y + rnd(-14, 14) * sc, h:rnd(40, 70) * sc, vx:rnd(-8, 8), vy:rnd(-8, 8), vh:rnd(25, 55),
                   life:rnd(.5, 1.1), color: Math.random() < .6 ? "#1a0f33" : (rI >= 4 ? mobRank(sh.data.rank).col : "#7a4ae0"), size:rnd(4, 8), g:0 });
    }
  ...
  for (const o of scene.children){
    if (o.userData.skyIsle){
      ...
      o.visible = !dungeon && currentThemeKey !== "throne";
      continue;
    }
    if (!o.userData.cloud) continue;
    o.visible = currentThemeKey !== "throne";
...
function syncHud(){
  ...
  $("rebirth").textContent = `Renacer ${P.rebirths}${P.tier ? ` · Amenaza ${P.tier}` : ""}`;
...
function panelStats(){
  const tabs = [["Stats","Atributos"],["Abyss","Abismo"],["Look","Aspecto"],["Mounts","Monturas"],["Class","Clase"],["Talents","Talentos"],["Codes","Códigos"]];
  ...
  if (panelTab === "Class"){
    ...
    return shell("Clase", nav + `<div class="spinbox">
        ...
        ${reel}
        <button class="btn gold big-w" id="spin-btn" ${P.spins > 0 && !spinning ? "" : "disabled"}>${spinning ? "Girando…" : P.spins > 0 ? "🎰 Girar la ruleta (1 giro)" : "Sin giros · consíguelos con códigos o en el modo infinito"}</button>
  ...
  if (panelTab === "Abyss") return panelAbyss(nav);
  if (panelTab === "Codes"){
...
modal.addEventListener("click", e => {
  ...
  if (t.id === "tier-btn"){ threatUnlock(); return; }
  if (t.dataset.tier !== undefined){ setThreat(+t.dataset.tier); return; }
  if (t.dataset.seal){ buySeal(t.dataset.seal); return; }
  if (t.dataset.class){
