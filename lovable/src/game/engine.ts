...
function panelItems(){
  ...
  const card = (o, i) => `<div class="card ${o.locked ? "lock" : ""}" style="--tc:${o.tc};--i:${i}"><span class="cg">${o.g}</span>
    <b>${o.name}</b><small>${o.sub}</small>${o.tag ? `<i class="ctier">${o.tag}</i>` : ""}</div>`;
  if (panelTab === "Beasts"){
  ...
  if (panelTab === "Relics"){
    ...
    body = all.map((r, i) => { const has = !!P.relics[r.id]; if (has) got++;
      return card({ locked:!has, tc: has ? "var(--gold)" : "var(--line)", g:r.glyph, name:r.name, tag: has ? "ACTIVA" : "🔒",
        sub: `${r.effect === "shadowDmg" ? `Sombras +${(r.value*100).toFixed(0)}%` : r.effect === "ariseLuck" ? `Arise +${r.value}%` : `Rango +${r.value}%`} · ${isleOf(r.isle)?.name || r.isle}` }, i); }).join("");
