...
const MOB_RANKS = [
  { r:"E", p:.40,  hp:1,   dmg:1,    loot:1,   sh:1,    arise:1,    col:"#9aa6b8" },
  { r:"D", p:.27,  hp:1.7, dmg:1.25, loot:1.7, sh:1.4,  arise:.92,  col:"#7ee07a" },
  { r:"C", p:.17,  hp:2.8, dmg:1.55, loot:2.8, sh:2,    arise:.82,  col:"#5ad2ff" },
  { r:"B", p:.10,  hp:4.5, dmg:1.9,  loot:4.5, sh:3,    arise:.7,   col:"#bb8cff" },
  { r:"A", p:.045, hp:7.5, dmg:2.4,  loot:8,   sh:4.6,  arise:.56,  col:"#ffd24a" },
  { r:"S", p:.015, hp:13,  dmg:3.2,  loot:15,  sh:7.5,  arise:.42,  col:"#ff5d6c" },
];
const mobRank = r => MOB_RANKS.find(x => x.r === r) || MOB_RANKS[0];
function rollMobRank(){
  // la suerte (hasta +100%) roba probabilidad a E y la reparte hacia arriba
  const luck = clamp((typeof ariseLuck === "function" ? ariseLuck() : 0) / 100, 0, 1);
  const w = MOB_RANKS.map((m, i) => i === 0 ? m.p * (1 - luck * .35) : m.p * (1 + luck * .35 * i / 2));
...
const baseDamage  = () => FORMULA.physicalDamage(weaponDmg(), P.stats.STR) * (blessed() ? 1.4 : 1) * (formActive() ? FORM.dmg : 1)
  ...
  * (1 + P.level * 0.012) * (now() < (player.dmgBuffUntil || 0) ? 1.3 : 1);
...
function spawnEnemy(def, x, y){
  ...
  e.elite = !e.boss && (e.rank === "A" || e.rank === "S");
  enemies.push(e);
  return e;
}
...
function killEnemy(e){
  ...
  if (!e.boss && MOB_RANKS.indexOf(R) >= 3){
    const gems = 3 * MOB_RANKS.indexOf(R) ** 2;
...
function playerConfig(){
  ...
  return {
    ...
    weapon: kitOf(P.weapon).color, weaponGlow: monarch || !!kitOf(P.weapon).glow,
...
function enemyConfig(e){
  ...
  const cfg = {
    body, features,
    scale: (e.def.r / 18) * (boss ? 1.75 : brute ? 1.25 : (L.height || 1)) * (1 + Math.max(0, MOB_RANKS.findIndex(m => m.r === e.rank)) * .05)
...
function render(dt){
  ...
  for (const e of enemies){
    ...
    if (e.elite && !v.userData.outline) addOutline(v, e.rank === "S" ? 0x6a0a14 : 0x6a4a08, 1.07);
...
function panelShadows(){
  ...
  const rk = x => MOB_RANKS.findIndex(m => m.r === (x.rank || "E"));
  const by = { dmg:(a, b) => shadowDmgOf(b) - shadowDmgOf(a), lvl:(a, b) => rk(b) - rk(a) || shadowDmgOf(b) - shadowDmgOf(a),
  ...
  const cards = list.map((s, i) => {
    const eq = P.squad.includes(s.uuid), can = false, R = mobRank(s.rank);
    return `<button class="card ${eq ? "eq" : ""} ${s.uuid === shadowSel ? "sel" : ""}" data-pick="${s.uuid}"
      ...
      <b>${s.name}</b><small>${fmt(shadowDmgOf(s))} de daño</small><i class="crank" style="--rc:${R.col}">${R.r}</i>
  ...
  if (sel){
    const eq = P.squad.includes(sel.uuid), can = false, SR = mobRank(sel.rank);
  ...
  return shell(`Sombras · ${P.squad.length}/${squadCap()}`, `
    ...
    Cada enemigo nace con un rango de <b>E</b> a <b>S</b>: cuanto más alto, más cuesta vencerlo y extraerlo, y más fuerte sale su sombra.</p>
    <div class="odds">${MOB_RANKS.map(m => `<span style="--rc:${m.col}"><b>${m.r}</b>${(m.p * 100).toFixed(m.p < .05 ? 1 : 0)}%</span>`).join("")}</div>`);
}
let shopSel = null;
function panelShop(){
...
function panelMap(){
  ...
  if (mapTab === "Regions"){
    ...
    right = `<div class="list">${Object.entries(DUNGEON_MODES).map(([id, m]) => `<div class="item"><span class="g">🌀</span>
  ...
  return shell("Mapa del mundo", `<div class="mapwrap2">
      <div class="mapside">${worldMapSVG()}
        ...
        <p class="hint">Toca un anillo para ver la región. El centro es Seúl; cada anillo es una región más dura.</p></div>
