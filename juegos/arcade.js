...
Arcade.admin = {
  ...
  activar(){ Almacen.escribir(this.CLAVE, '1'); this.montar(); this.montarBoton(); },
  desactivar(){
  ...
  montar(){
    ...
    this.montarBoton();
    if (document.querySelector('.capaAdmin')) return;
    ...
    capa.innerHTML =
      '<div class="cajaAdmin" role="dialog" aria-label="Consola de administración">' +
        ...
        '</div>' +
        '<div class="pieAdmin">Tab autocompleta · ↑ ↓ historial · Esc cierra · F9 o Ctrl+Shift+A la abren desde cualquier sitio</div>' +
      '</div>';
  ...
  /**
   * Botón visible en la barra de cada pantalla. Antes la consola solo se abría
   * con F9, y en muchos teclados de portátil o en el móvil esa tecla no está:
   * el modo administrador quedaba encendido pero sin manera de usarlo.
   */
  montarBoton(){
    if (!this.activo()) return;
    document.querySelectorAll('.barra, #barraSuperior, [id^="barraSuperior"]').forEach(barra => {
      if (barra.querySelector('.btnAdmin')) return;
      const b = document.createElement('button');
      b.className = 'btnAdmin';
      b.type = 'button';
      b.title = 'Consola del creador (F9)';
      b.setAttribute('aria-label', 'Abrir la consola del creador');
      b.textContent = '👑';
...
addEventListener('keydown', ev => {
  // Dos atajos: F9, y Ctrl/Cmd+Shift+A para los teclados sin tecla de función
  if (ev.key === 'F9' ||
